...
export function SourceListWithRecommendations() {
  ...
  // Default tracked tools/categories for recommendations
  const trackedTools = ["Claude Code", "Cursor", "Copilot", "Windsurf", "Aider", "Cline"];
  const trackedCategories = ["Code Generation", "Code Review", "Testing", "Debugging", "DevOps", "Architecture"];
            ...
            <Card key={source.id} className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {source.errorCount > 2 && <Badge variant="breaking">Unhealthy</Badge>}
                  {!source.enabled && <Badge variant="superseded">Disabled</Badge>}
                </div>
